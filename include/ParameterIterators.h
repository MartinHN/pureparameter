...
#ifndef PMCPP_PARAMETERITERATORS_H
#define PMCPP_PARAMETERITERATORS_H
...
namespace ParamIterators{
    struct Up_Group{
        ...
        ParameterGroup * p;
        bool next(){bool res= (p!=nullptr);if(res)p=p->parentGroup;return res;}
    };
    struct Up_Node{
        Up_Node(ParameterNode* _p,ParameterNode* _toRoot=nullptr):p(_p),toRoot(_toRoot){}
        ParameterNode * p;
        ParameterNode* toRoot;
        ParameterNode* next(){ParameterNode* cur= p;p=(!p || p==toRoot)?nullptr:p->parentGroup;return cur;}
    };
    struct FromRoot{
    ...
    public:
        ...
        ParameterGroup * next(){
            ...
            if(current->paramGroups.size() && !hasReachedMaxDepth()){
                //go deeper leftmost while we can
                poses.push_back(0);
                current = current->paramGroups[0];
                return current;
            }
            else{
                // go up until unvisited
                while(poses.size() && current->parentGroup){
                    int nextI = poses.back()+1;
                    auto curList = current->parentGroup->paramGroups;
...
#endif //PMCPP_PARAMETERITERATORS_H
